...
마지막 기준 상태: Part I. 1장 「출애굽」, Part I. 2장 「광야」 구현·라우팅·이미지 반영 단계 완료. Part I. 3장 「여리고」는 story data, 엔딩, 런타임 등록, 홈 카드 동적 활성화 구조가 반영되었으며 최종 QA 준비 단계이다.
...
상태: 플레이 가능, 최종 QA 대상. 시내산 언약과 금송아지 사건까지 확장됨.
...
상태: 테스트 플레이 가능 구조 반영, 최종 QA 대상. 완료 판정 전 validator와 브라우저 QA가 필요함.
...
- `TOTAL_PROGRESS = 18`
- 시작 노드: `jericho_01_jordan_edge`
- 최종 노드: `jericho_18_memory_after_fall`

이미지 상태:

- 플레이 장면 이미지 경로: `assets/images/story/jericho/play_left_520x650/`
- 엔딩 이미지 경로: `assets/images/story/jericho/original_16x9/`
- 최종 완료 판정 전 18개 플레이 노드와 7개 엔딩 이미지의 404 여부를 확인해야 한다.
...
3. 여리고는 `chapter-runtime.js`에서 `jerichoStructurePatch.js`와 `jerichoEndingsPatch.js`를 동적으로 확인하고, 필수 데이터가 모두 있을 때 홈 카드가 플레이 가능 상태로 전환된다.
4. 장기적으로는 `main.js`에 `startChapter(chapterId)`, `getPlayArtBase(node)`, `getEndingArtBase(profile)` 구조를 정식 반영하는 것이 이상적이다.
5. 단기적으로는 `chapter-runtime.js`를 통해 다중 챕터 구조를 안정적으로 유지한다.
...
Part I. 1장 「출애굽」, Part I. 2장 「광야」, Part I. 3장 「여리고」는 최종 완료 판정 전에 반드시 아래를 확인해야 한다.
...
4. 홈 → 광야 진입 확인.
5. 홈 → 여리고 진입 확인.
6. 출애굽 주요 배드엔딩 및 최종 엔딩 진입 확인.
7. 광야 1–26번 노드 진행 확인.
8. 여리고 1–18번 노드 진행 확인.
9. 광야 전반부 이미지 표시 확인.
10. 광야 후반부 시내산/금송아지 이미지 표시 확인.
11. 광야 확장 엔딩 7개 이미지 표시 확인.
12. 여리고 플레이 이미지 18개 표시 확인.
13. 여리고 엔딩 이미지 7개 표시 확인.
14. 콘솔 404 오류 확인.
15. Part II–VI 카드가 아직 플레이로 진입하지 않는지 확인.
16. 홈 기능 패널 동행자/엔딩/설정이 깨지지 않았는지 확인.
...
Part I. 3장 여리고: story data, 엔딩, 런타임 등록, 홈 카드 동적 활성화 구조 반영 완료, 최종 QA 대상.
홈 화면: Part I–VI 탐색 구조 구현 완료.
챕터 런타임: 다중 챕터 구조 1차 적용 완료.
다음 구현 후보: Part I. 4장 사사 시대.
```
